...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    NSString *recipeCellID = @"RecipeCell";
    UITableViewCell *cell = [self.tableView dequeueReusableCellWithIdentifier:recipeCellID];
    if(cell==nil){
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender{
    if([segue.identifier isEqualToString:@"showRecipeDetail"]){
        ...
        RecipeDetailViewController *destView = (RecipeDetailViewController*) segue.destinationViewController;
        if([self.searchDisplayController isActive]){
            indexPath=[self.searchDisplayController.searchResultsTableView indexPathForSelectedRow];
             destView.recipe=[searchResults objectAtIndex:indexPath.row];
        }
        else{
            indexPath=[self.tableView indexPathForSelectedRow];
             destView.recipe=[recipes objectAtIndex:indexPath.row];
        }
        //AN THANH TAB BAR PHIA DUOI DI NAO
        [destView setHidesBottomBarWhenPushed:YES];
...
-(BOOL)searchDisplayController:(UISearchDisplayController *)controller shouldReloadTableForSearchString:(NSString *)searchString{
    ...
    NSString *scope = [self.searchDisplayController.searchBar.scopeButtonTitles objectAtIndex:selectedIndex];
